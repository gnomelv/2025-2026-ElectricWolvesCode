...
@Disabled
@Autonomous(name = "BLUE_CLOSE", group = "Autonomous")
public class Meet_2_BLUECLOSE extends OpMode {
   ...
   private boolean shotsTriggered = false;
    ...
    private final Pose startPose = new Pose(24, 127, Math.toRadians(-37));
    private final Pose closeScore = new Pose(60, 95, Math.toRadians(-40));
    private final Pose PPG_pickup = new Pose(50, 85, Math.toRadians(-180));
    private final Pose PPG_end = new Pose(15, 85, Math.toRadians(-180));
    private PathChain start_to_farshoot, farshoot_to_PPGPickup, PPGPickup, PPGEnd_to_Farshoot, LEAVETOSCORE;
    ...
    public void buildPaths() {
        start_to_farshoot = follower.pathBuilder()
                .addPath(new BezierLine(startPose, closeScore))
                .setLinearHeadingInterpolation(startPose.getHeading(), closeScore.getHeading())
                .build();

        farshoot_to_PPGPickup = follower.pathBuilder()
                .addPath(new BezierLine(closeScore, PPG_pickup))
                .setLinearHeadingInterpolation(closeScore.getHeading(), PPG_pickup.getHeading())
                .build();
        PPGPickup = follower.pathBuilder()
        ...
        PPGEnd_to_Farshoot = follower.pathBuilder()
                .addPath(new BezierLine(PPG_end, closeScore))
                .setLinearHeadingInterpolation(PPG_end.getHeading(), closeScore.getHeading())
                .build();

        LEAVETOSCORE = follower.pathBuilder()
                .addPath(new BezierLine(closeScore, PPG_end))
                .setLinearHeadingInterpolation(closeScore.getHeading(), PPG_end.getHeading())
                .build();
    }
    ...
    public void statePathUpdate(){
        ...
        switch(THESTATEOFTHEPATH){
            ...
            case SHOOT_farshoot_1:
                if (!follower.isBusy()){
                    if (!shotsTriggered){
                        gamespecific.fireShots(3, false);
                        shotsTriggered = true;
                    }
                    else if (shotsTriggered && !gamespecific.isShooting()){
                        shotsTriggered = false;
                        follower.followPath(farshoot_to_PPGPickup, true);
            ...
            case DRIVE_farshoot_to_GPPPICKUP:
                if (!follower.isBusy()){
                    ...
                    follower.followPath(PPGPickup, 0.4,true);
            ...
            case DRIVEINTAKE_GPP:
                ...
                if (!follower.isBusy()){
                    gamespecific.powerIntake();
                    follower.followPath(PPGEnd_to_Farshoot, true);
            ...
            case GPPEND_to_farshoot:
                ...
                if (!follower.isBusy()){
                        if (!shotsTriggered){
                            ...
                            shotsTriggered = false;
                            follower.followPath(LEAVETOSCORE);
